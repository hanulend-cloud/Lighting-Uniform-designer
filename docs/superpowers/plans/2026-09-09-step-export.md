...
- L5 "오목"(`dir: '오목'`)은 이번 범위에서 Boolean Cut을 구현하지 않았다(설계 문서에 명시된 범위 밖 — "돌출"만 지원). 필요하면 별도 태스크로 추가. (사용자가 이 설정으로 내보내면 확인 다이얼로그로 경고하도록 최종 리뷰에서 추가함 — `step-ui.js`)
- `BRepAlgoAPI_Fuse`를 수천 번 순차 호출하는 구조라, 정말 수천 개 돌기가 있으면 여전히 느릴 수 있다(설계 문서에서 이미 사용자에게 안내한 리스크). 실측: LED가 많기만 해도(L5 없이, 300개+) 브라우저에서 STEP 완성까지 15분을 넘길 수 있음(정상 동작, 단지 느림 — 콘솔 에러·정지 없이 계속 진행되는 것을 확인함).
- `oc-build.js`의 `triFace`/`sewToSolid`/`coneBump`/`ridgeBump`가 만드는 점·면 스크래치 객체는 `.delete()`하지 않는다(Task 2 리뷰에서 발견, "고부피 호출부(바디 메쉬·L5 돌기)에서 재검토" 하기로 의도적으로 미룸 — 이후 태스크에서도 리트로핏하지 않음, 최종 리뷰에서도 동일하게 확인·유지). 현재는 내보내기가 항상 1회용 Worker(`step-ui.js`)에서 실행되고 끝나면 `terminate()`되어 WASM 힙째로 회수되므로 실제 위험은 낮지만, 이 함수들을 워커 밖(예: 오래 사는 프로세스에서 반복 호출)에서 재사용할 경우엔 반드시 정리 코드를 추가해야 한다.

## 최종 리뷰에서 추가로 수정한 것 (8개 태스크 완료 후)

1. L5 "오목" 내보내기 시 확인 다이얼로그로 경고(`step-ui.js`) — 위 리스크 항목 참고.
2. `step-ui.js`가 `geom.levelText`를 정규식으로 파싱해 활성 레벨을 재구성하던 것을 제거 — `active`(이미 `main.js`에서 넘어오는 실제 배열)를 그대로 `postMessage`에 실어 `step-worker.js` → `buildStepForSpec`까지 전달(`buildStepForSpec`의 새 6번째 인자 `activeIn`). `test/smoke-step.mjs`는 이 인자를 안 넘기므로 기존처럼 `spec.levels[l].on` 기반 폴백 경로를 그대로 탄다.
3. `README.md`에 STEP 내보내기 기능 한 줄, `src/export/` 구조 한 줄 추가.
